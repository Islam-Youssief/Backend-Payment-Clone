...
import api.core.exceptions as api_exception
...
class TestGetService(unittest.TestCase):
    ...
    def test_returns_request_wiremock_when_env_is_not_lc(self):
...
class TestRequestWrapper(unittest.TestCase):
    ...
    def test_request_calls_request_with_expected_params(self):
        self.wrapper.request(method="GET",url="fake_url",headers={'X-Test-Header':'fake value'})
    ...
    def test_request_raise_response_error_if_response_is_not_ok(self):
        ...
        with self.assertRaises(api_exception.ResponseError) as exc:
...
class TestWiremockRequester(unittest.TestCase):
    def test_request_calls_requests_with_expected_params(self):
        self.test_request = requests_doubles.RequestSenderDouble()
        wrapper = api_requests.WiremockRequester(self.test_request)
        wrapper.request(method="GET",url="fake_url", headers={'X-Test-Header': 'fake value'})
        self.test_request.assert_that_request_is_called_with(
            method='GET',
